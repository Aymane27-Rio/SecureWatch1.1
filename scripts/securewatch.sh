...
# 1) Run modules (each module writes NDJSON entries or echoes JSON we capture)
...
# 2) Parse system logs for suspicious activity (SSH brute force, sudo anomalies)
...
# 5) Alert on critical findings (severity >= 3)
